...
{
	private Command attacksurface = new Command()
	{
			...
			try
			{
				...
				if (shared != null)
					attackSurface += "shared user-id = "
							+ currentSession.applicationContext
									.getPackageManager().getPackageInfo(
											packageName, 0).applicationInfo.uid
							+ " (" + shared + ")\n";
			...
			for (PackageInfo currentPackage : packages)
			{
						...
						|| (!bothFiltersPresent && noFilters))
				{
					...
					if (librariesString.length() > 0)
						currentSession.send("Shared libraries: "
								+ librariesString + "\n", true);
	...
	private Command path = new Command()
	{
	...
	private Command shareduid = new Command()
	{
	...
	public Packages()
	{
		...
		commands.put("attacksurface", attacksurface);
		commands.put("info", info);
		commands.put("path", path);
		commands.put("shareduid", shareduid);
	}
}
